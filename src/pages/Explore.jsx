...
import {
  faFilter, faSearch, faDollarSign, faClock, faStar,
  faArrowRight, faTimes, faChevronDown,
  ...
  return (
    <div className="pt-24 min-h-screen bg-dark-900">
      <div className="max-w-[1600px] mx-auto px-6 pb-12 flex flex-col lg:flex-row gap-8">
        ...
        {/* Sidebar Filters */}
        <aside className={`lg:w-80 flex-shrink-0 transition-all duration-300 ${isSidebarOpen ? 'block' : 'hidden lg:block'}`}>
            ...
            <div className="space-y-8">
              ...
              <div>
                <div className="flex justify-between mb-4">
                  <label className="text-sm font-semibold text-white">Max Monthly Income</label>
                  <span className="text-primary-light font-bold">${filters.income.toLocaleString()}</span>
                </div>
                <input
              ...
              <div>
                <label className="text-sm font-semibold text-white block mb-3">Min Visa Duration</label>
                <div className="relative">
                  ...
                  <FontAwesomeIcon icon={faChevronDown} className="absolute right-4 top-4 text-slate-500 pointer-events-none text-xs" />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between mb-4">
                  <label className="text-sm font-semibold text-white">Max Cost of Living</label>
                  <span className="text-primary-light font-bold">{filters.cost}</span>
                </div>
                <input
                  type="range" min="20" max="100" step="5"
                  value={filters.cost}
                  onChange={(e) => updateFilter('cost', parseInt(e.target.value))}
                  className="w-full h-1.5 rounded-lg cursor-pointer"
                />
                <div className="flex justify-between mt-2 text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                  <span>Cheap</span>
              ...
              <div>
                <div className="flex justify-between mb-4">
                  <label className="text-sm font-semibold text-white">Min Quality of Life</label>
                  <span className="text-primary-light font-bold">{filters.qol}+</span>
                </div>
              ...
              {/* Processing Time */}
              <div>
                <label className="text-sm font-semibold text-white block mb-3">Processing Speed</label>
        ...
        <main className="flex-grow">
          ...
          <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="relative w-full md:w-96">
              <FontAwesomeIcon icon={faSearch} className="absolute left-4 top-4 text-slate-500" />
              <input
                type="text"
              ...
              {loading ? (
                ...
                >
                  <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4">
                    <FontAwesomeIcon icon={faTimes} className="text-slate-600 text-3xl" />
                  </div>
                  <h3 className="text-xl font-bold text-white mb-2">No countries found</h3>
                  <p className="text-slate-400 max-w-xs mx-auto">Try adjusting your filters to see more results.</p>
                  <button
                    onClick={resetFilters}
      ...
      <div className="p-6 flex-grow">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-slate-500 text-xs font-semibold uppercase tracking-wider">{country.visaName}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-white/5 p-3 rounded-2xl">
            ...
            <div className="text-white font-bold">
              {country.currencySymbol}{country.minIncomeMonthly.toLocaleString()}
            </div>
          </div>
          <div className="bg-white/5 p-3 rounded-2xl">
            <div className="flex items-center gap-2 text-slate-500 text-[10px] font-bold uppercase mb-1">
            ...
            <div className="text-white font-bold">{country.durationMonths} Months</div>
          </div>
        </div>
        ...
        <div className="space-y-2 mb-6">
          <div className="flex justify-between text-xs">
            ...
            <span className="text-white font-medium">{country.costOfLivingIndex} Index</span>
          </div>
      ...
      <div className="p-4 bg-white/5 border-t border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full ${
            country.difficulty === 'Easy' ? 'bg-green-400'
            : country.difficulty === 'Medium' ? 'bg-yellow-400'
            : 'bg-red-400'
          }`} />
          <span className="text-xs text-slate-300 font-medium">{country.difficulty}</span>
        </div>
        <div className="flex items-center gap-3">
          <Link
            to={`/compare?add=${country.id}`}
