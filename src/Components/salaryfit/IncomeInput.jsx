...
export default function IncomeInput({ value, onChange, onSubmit, error }) {
  ...
  <div className="flex-1 relative">
    <div className="flex items-center bg-dark-900 border border-white/15 rounded-xl overflow-hidden focus-within:border-primary transition-colors">
      ...
      <span className="px-4 text-slate-400 font-bold text-lg select-none border-r border-white/10 h-12 md:h-14 flex items-center">
        $
      </span>
      ...
      <input
        ...
        className="flex-1 bg-transparent text-white text-base md:text-lg font-bold px-4 h-12 md:h-14 focus:outline-none placeholder:text-slate-600"
      />
    </div>
  ...
  <button
    type="submit"
    className="h-12 md:h-14 px-6 md:px-7 rounded-xl bg-primary hover:bg-primary-dark text-white font-bold text-sm transition-all shadow-lg hover:shadow-primary/40 flex items-center justify-center gap-2 whitespace-nowrap"
  >
